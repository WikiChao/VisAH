...
from .demucs.demucs import DConv, rescale_module
...
class HDemucsRemixer(nn.Module):
    """
    ...
    def forward(self, mix, target, context=None):
        x = mix
        length = x.shape[-1]

        z = self._spec(mix)
        target_z = self._spec(target)

        if not self.magphase:
            mag = self._magnitude(z).to(mix.device)
            mag_target = self._magnitude(target_z).to(mix.device)
        else:
            mag, phase = self._magphase(z)
            mag = mag.to(mix.device)
            phase = phase.to(mix.device)
            mag_target, phase_target = self._magphase(target_z)
            mag_target = mag_target.to(mix.device)
            phase_target = phase_target.to(mix.device)
        ...
        if self.latent_transformer is not None:
            x = self.latent_transformer(x, context)
        ...
        # # if masking, you don't need this
        ...
        if not self.magphase:
            ...
            mag_out = mag[:, None] * x
            zout = mag_out * torch.exp(1j * phase[:, None]) 
        # print range 
        ...
        if self.magphase:
            return x[:, 0], mag_target, mag_out[:, 0], 1
        else:
            return x[:, 0], target_z, zout[:, 0], 1

# unit test
if __name__ == "__main__":
    from attention import ZeroTransformer
    latent_transformer = ZeroTransformer(768, 16, 32, depth=6, context_dim=768)
    context = torch.rand(2, 10, 768)
    model = HDemucsRemixer(sources=["output"], audio_channels=2, latent_transformer=latent_transformer)
    mix = torch.rand(2, 2, 44100*10)
    out = model(mix, target=mix, context=context)
    print(out[0].shape)